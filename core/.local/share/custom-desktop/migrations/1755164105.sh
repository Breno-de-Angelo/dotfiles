...
if omarchy-cmd-present chromium; then
  set_theme_colors() {
    if [[ -f ~/.config/custom-desktop/current/theme/chromium.theme ]]; then
      chromium --no-startup-window --set-theme-color="$(<~/.config/custom-desktop/current/theme/chromium.theme)"
    else
